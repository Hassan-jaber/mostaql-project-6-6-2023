...
function calculateHijriCalendar() {
  ...
  const currentDate = moment().format('iDD/iMM');
  const currentDateFull = moment().format('DD/MM/YYYY');
  ...
  const elapsedDays = moment(currentDateFull, 'DD/MM/YYYY').diff(startOfYear, 'days');
  const remainingDays = endOfYear.diff(moment(currentDateFull, 'DD/MM/YYYY'), 'days');
  ...
  // Update the element with the current Hijri date in the format 'dd/mm'
  const currentDateElement = document.querySelector(".currentDate-element");
